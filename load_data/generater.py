...
class GeneralizedDataset:
    ...
    def check_dataset(self, checked_id_file):
        """
        use multithreads to accelrate the process
        check the datset to avoid some problems listed in method '_check'
        """

        if os.path.exists(checked_id_file):
            info = [line.stip().split(", ") for line in open(checked_id_file)]
        ...
        tasks = [executor.submit(self._chuck, seq.tolist()) for seq in seqs]
